      ...
      if (threadIdx.x %64 ==0 ){
          ...
          for (int k=0; k< K; k++){
            int min_idx = threadIdx.x ;
              ...
              K_ele[k]= sharedMemory[min_idx];
              K_class[k] = sharedMemory_class[min_idx];
              sharedMemory[min_idx] = 100000;

          }
          for (int k=0; k< K; k++){
            sharedMemory[(threadIdx.x )+k]= K_ele[k];
            sharedMemory_class[(threadIdx.x )+k] = K_class[k];
          }
      ...
      for (int s = blockDim.x/128; s > 0; s >>= 1)
    	{
      		...
      		if (threadIdx.x < s && s== 2){
              if(blockIdx.x == 0){
                //printf("s,%d threadIdx %d ", s, threadIdx.x * 128   );
              }
              ...
              for (int k=0; k< K; k++){
                  ...
                  for(int idx= (threadIdx.x * 128)+ 64; idx < ((threadIdx.x * 128)+ 64) + K; idx ++)
                  {
                    if(sharedMemory[idx] < sharedMemory[min_idx]){
                      min_idx = idx;
                    }
          ...
          if (threadIdx.x < s && s== 1){
            ...
            for (int k=0; k< K; k++){
                ...
                for(int idx= (threadIdx.x * 128)+ 128; idx < ((threadIdx.x * 128)+ 128) + K; idx ++)
                {
                  if(sharedMemory[idx] < sharedMemory[min_idx]){
                    min_idx = idx;
                  }
